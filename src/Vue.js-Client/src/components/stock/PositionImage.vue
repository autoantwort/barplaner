<template>
  <div v-if="realPosition !==null && realPosition.image && realPosition.image !== null" style="position: relative;width:100%;" v-bind:style="{  'max-height': maxHeight + 'px' }">
    <div ref="horizontal" style="height:1px; width: 100%; background:red;position: absolute;" v-bind:style="{top: relativeYPosition}"></div>
    <div ref="vertical" style="width:1px; height: 100%; background:red;position: absolute;" v-bind:style="{left: relativeXPosition}"></div>
    <img
      ref="img"
  ...
  methods: {
    updateRealPosition() {
      if (this.position === null) {
        console.error(
          "PositionImage.vue: position is null, must be a number (id) or a position object"
        );
        ...
        if (
          this.realPosition.imageId !== null &&
          !this.realPosition.image !== undefined
        ) {
          this.retrieveImage();
        }
      }
    ...
    updateRelativePosition() {
      ...
      if (this.realPosition !== null && image !== undefined) {
        ...
        if (
          ...
          !(image.naturalWidth > image.width && image.height < 500)
        ) {
          // compute left offset:

          const realImageWidth = Math.round(
            (image.naturalWidth / image.naturalHeight) * 500
          );
          ...
          this.relativeXPosition =
            (this.realPosition.xPositionOnImage *
          ...
          this.relativeXPosition =
            this.realPosition.xPositionOnImage * 100 + "%";
        }
