    ...
    .then((role) => {
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          ...
          permission: roleName,
          description:
            "The Access token to access the facebook graph api to get the events from the symposion page.",
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          ...
          permission: roleName,
          description:
            "A boolean, if true, the system copy facebook events to the barplaner, if false, nothing get copied.",
          value: true,
        },
      })
        .then((s) => {
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          ...
          permission: roleName,
          description:
            "The intervall, in wich the server will fetch the facebook graph api for new events.",
      ...
      AccessToken.reload().then(() => {
        PageID.reload().then(() => {
          axios
            .get(
              PageID.value +
                "/events?fields=start_time,end_time,description,id,interested_count,is_canceled,is_draft,maybe_count,name,declined_count,noreply_count,type,cover&include_canceled=true&access_token=" +
                AccessToken.value
            )
            .then((response) => {
      ...
      .then((events) => {
        ...
        Bar.findAll({
          ...
          .then((bars) => {
            ...
            Setting.findByPk("defaultNumberOfPersonsToClean")
              .then((numberSetting) => {
                let barsToCreate = [];
                ...
                for (
                  let b = 0;
                  e < events.length && b < bars.length && events[e].start > now;

                ) {
                  // check if on same date:
                  if (
                    bars[b].start.getFullYear() ===
                      events[e].start.getFullYear() &&
                    ...
                    // event is older then a bar => create bar
