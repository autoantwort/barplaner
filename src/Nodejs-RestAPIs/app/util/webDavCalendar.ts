      ...
      client.saveBar = (bar) => {
        ...
        for (const e of calendar.objects) {
          if (e.url.endsWith("bar" + bar.id)) {
            e.calendarData = data;
            client
              .updateCalendarObject(e)
              .catch((e) =>
                console.error("Can not update Calendar object: ", e)
              );
            return;
          }
        }
        ...
        client
          .createCalendarObject(calendar, { data, filename: "bar" + bar.id })
          .then((co) => calendar.objects.push(co))
          .catch((e) =>
            console.error("Can not create Calendar object: ", e, data, bar)
          );
      };
      db.addSyncCallback(() => {
