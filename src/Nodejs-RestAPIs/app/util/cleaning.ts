...
export const computeCleaning = (barID, numberOfPersons) => {
  return new Promise((resolve, reject) => {
    ...
    Bar.findByPk(barID)
      .then((bar) => {
        //console.log("---------------", ++what, "---------------");
        User.findAll({ where: { active: true }, raw: true })
          .then((users) => {
            ...
            let wieOftNicht = Math.round(
              ((users.length / numberOfPersons) * 3) / 7
            );
                ...
                BarDuty.findAll({
                  ...
                  .then((duties) => {
                      ...
                      if (duty.have_to_clean && duty.barID === barID) {
                        ...
                        for (const user of users) {
                          if (user.id === duty.userID) {
                            experienced_cleaner_present =
                              user.experienced_cleaner ||
                              experienced_cleaner_present;
                            break;
                          }
                        }
                    ...
                    for (let i = 0; i < users.length; ++i) {
                      ...
                      if (
                        duties.some(
                          (r) => r.have_to_clean && r.userID === users[i].id
                        )
                      )
                        ...
                        attributes: [
                          ...
                          [
                            sequelize.Sequelize.fn(
                              "count",
                              sequelize.Sequelize.col("userID")
                            ),
                            "count",
                          ],
                        ],
                            ...
                            .then((barCleanedCount) => {
                              ...
                              for (let i = 0; i < users.length; ++i) {
                                ...
                                // haben noch nie geputzt
                                  ...
                                  ver[users[i].id].cleaned === undefined
                                ) {
                                  users[i].cleaned = 0;
            ...
            BarDuty.findAll({
              ...
              .then((barCleanedCount) => {
                ...
                Setting.findByPk("defaultNumberOfPersonsToClean")
                  .then((numberSetting) => {
                    let wieOftNicht = Math.round(
                      ((users.length / Number(numberSetting.value)) * 3) / 7
                    );
