...
db.addSyncCallback(() => {
  Role.findCreateFind({
    ...
    defaults: {
      ...
      description:
        "You can change settings that have something to do with the newsletter.",
    },
  })
    .then((role) => {
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          name: "sendDaysBeforeBar",
          description:
            "The number of days before the bar at which the newsletter is sent. Example: The bar on 15.10 and this value is 2, the newsletter will be sent on 13.10.",
          value: "2",
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          name: "templateNewsletterId",
          description:
            "The id of the newsletter that should be used as template.",
          value: "-1",
          permission: NewsletterAdminRole.name,
        },
      })
      ...
      Setting.findCreateFind({
        ...
        defaults: {
          name: "DefaultImageURL",
          description:
            "The url to the image that should be used, if a bar has no cover image (created in the orga system and not on facebook).",
          value:
      ...
      .then((days) => {
        SendTime.reload()
          .then(() => {
            ...
            if (result === null) {
              reject(
                "Setting newsletterSendTime: Not a valid value: " +
                  SendTime.value
              );
            } else {
...
function replacePlaceholders(text, barObject, defaultImageURL) {
  ...
  const date =
    barObject.start.getDate() + "." + (barObject.start.getMonth() + 1);
  ...
  const coverImageURL = barObject.facebookCoverImageURL
    ? barObject.facebookCoverImageURL
    : defaultImageURL;
  ...
  text = text.replace(
    /https:\/\/www\.hilton\.rwth-aachen\.de\/.+\/coverImageURL\.png/g,
    coverImageURL
  );
  return text;
}
...
function sendNewsletter(barObject, newsletterId, sendTime) {
  ...
  Wordpress.query("Select * from wp_2_newsletter_emails where id = ?", {
    replacements: [newsletterId],
    ...
    .then((rows) => {
      if (rows.length === 1) {
        ...
        Wordpress.query(
          "Select id from wp_2_newsletter_emails where barId = ?",
          { replacements: [barObject.id], type: QueryTypes.SELECT }
        )
          .then((bars) => {
            DefaultImageURL.reload()
              .then(() => {
                ...
                if (bars.length === 0) {
                  ...
                  template.message = replacePlaceholders(
                    template.message,
                    barObject,
                    DefaultImageURL.value
                  );
                  template.subject = replacePlaceholders(
                    template.subject,
                    barObject,
                    DefaultImageURL.value
                  );
                  ...
                  sql +=
                    "(" + Object.getOwnPropertyNames(template).join(", ") + ")";
                  sql += " VALUES ";
                  sql +=
                    "(" +
                  ...
                  for (let nletter of bars) {
                    let newsletter: any = nletter;
                    ...
                    if (newsletter.status !== "sent") {
                      if (barObject.canceled) {
                        ...
                        let sql =
                          "Delete from wp_2_newsletter_emails WHERE id = ?";
                        Wordpress.query(sql, {
                          replacements: [newsletter.id],
                      ...
                      sql += Object.getOwnPropertyNames(update)
                        .map((p) => p + " = :" + p)
        ...
        console.error(
          "There is no newsletter for the given template newsletter id ",
          newsletterId
        );
      }
    })
