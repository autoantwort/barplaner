...
let convertFileEnding = new RegExp(
  "\\.(" + env.gitLabBrowser.convertedToPdf.join("|") + ")$"
);
...
Telegram.bot.onText(/\/(files?|datei(en)?|pdfs?|git|gitlab)/, (msg) => {
  ...
  const message = fileBrowserMessageCreator.createMessage(
    fakeUser,
    "Wähle eine Datei oder ein Verzeichnis aus. Nutze `..` um ein Verzeichnis aufzusteigen."
  );
  // see https://docs.gitlab.com/ee/api/repositories.html#list-repository-tree
  axios
    .get("/projects/" + env.gitLabBrowser.projectId + "/repository/tree")
    .then((res) => {
      message.addButtonToRow("Schließen", State.CLOSE, "-1");
      message.newRow();
      for (let entry of res.data) {
        message.addButtonToRow(
          (entry.type === "tree" ? "↳ " : "⤓ ") + entry.path,
          entry.type === "tree" ? State.DIR : State.FILE,
          addDataMapping({ path: entry.path, parent: 0, sha: entry.id })
        );
        message.newRow();
      }
...
const fileBrowserMessageCreator = Telegram.registerResponseSystem(
  "gitBrowser",
  (message) => {
    if (message.state === State.CLOSE) {
      Telegram.deleteTelegramMessage(
        message.chatId,
        message.messageId,
        new Date()
      );
    ...
    if (message.state === State.DIR) {
      ...
      message.state === State.FILE ||
      message.state === State.ORIGINAL
    ) {
      if (
        message.state !== State.ORIGINAL &&
        convertFileEnding.test(data.path)
      ) {
        // If a pdf is available, ask if the user want the pdf
        message.addButtonToRow("Abbrechen", State.CLOSE, "-1");
        message.newRow();
        message.addButtonToRow(
          "Download as " + data.path.substring(data.path.lastIndexOf(".") + 1),
          State.ORIGINAL,
          message.data
        );
        ...
        axios
          ...
          .then((res) => {
            Telegram.bot
              .sendDocument(message.chatId, res.data, {}, fileOptions)
              .catch(console.error);
            Telegram.deleteTelegramMessage(
              message.chatId,
              message.messageId,
      ...
      const path =
        (data.path.endsWith(".md")
          ? data.path
          : data.path.substring(0, data.path.lastIndexOf("."))) + ".pdf";
