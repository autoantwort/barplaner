...
const sendCronJob = new CronJob(
  "00 * " + env.webNotifications.sendTime + " * * *",
  function () {
    const now = new Date();
    Bar.findAll({
      where: {
      ...
      .then((bars) => {
        for (const bar of bars) {
          if (
            env.webNotifications.sendDaysBefore.indexOf(
              getDayDiff(now, bar)
            ) !== -1
          ) {
  ...
  WebPushSubs.findAll()
    .then((subs) => {
      for (const sub of subs) {
        webPush
          .sendNotification(JSON.parse(sub.subscription), data)
          .catch(() => {
