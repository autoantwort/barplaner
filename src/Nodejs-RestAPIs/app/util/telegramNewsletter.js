...
const createCronJob = value => {
    cronJobs[value.chatId] = new CronJob('00 ' + value.sendAt.substring(2, 4) + ' ' + value.sendAt.substring(0, 2) + ' * * *', function() {
        TelegramNewsletter.findByPk(value.chatId).then(value => {
            ...
            Bar.findAll({
                ...
                bars.filter(b => b.description /* send only when the bar has a description*/ )
                    .filter(b => {
