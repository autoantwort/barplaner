  ...
  <b-card no-body class="border-info">
    ...
    <b-card-footer class="bg-info border-info">
      <template v-if="usedForItem"
        >Every item (e.g. banana juice from one brand) can have a specific position. If a item has no position, the position of the item group (e.g. banana
        juice) is used.</template
      >
      <template v-else
        >Every item group can have a position. If a item group has no position, all items in the group has no position by default. Items can have a own
        position.</template
      >
    </b-card-footer>
  </b-card>
