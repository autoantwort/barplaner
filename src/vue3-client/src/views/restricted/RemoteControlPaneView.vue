  ...
  <div class="container">
    <div class="row mt-2">
      <div class="col-12">
        ...
        <div v-if="cantMoveCount > 0" class="alert alert-warning text-center" role="alert">
          In some control panels you don't have the rights to move the items
        </div>
        <div v-for="(client, key) in clients" :key="key">
          ...
          >
            <div
              ...
              :key="ikey"
              v-bind:style="{
                left: (mobilePositions ? item.mobileX : item.x) * 70 + 'px',
                top: (mobilePositions ? item.mobileY : item.y) * 70 + 'px',
                height: (item.type === 'DIMMER_GROUP') * 210 + 70 + 'px',
                width: (item.type !== 'DIMMER_GROUP') * 140 + 70 + 'px',
              }"
              :id="key + ikey + 'Root'"
  ...
  methods: {
    ...
    handleTouchStart(e, item) {
      if (this.enableMove) {
        ...
        if (target.parentElement.getAttribute('canMoveItems') !== 'true') return;
    ...
    handleDragStart(e, item) {
      if (e.target.parentElement.getAttribute('canMoveItems') !== 'true') return;
    ...
    handleDragOver(e) {
      ...
      this.webSocket.send(this.draggedItemPath + xName + ':' + (x / 70).toFixed());
      this.webSocket.send(this.draggedItemPath + yName + ':' + (y / 70).toFixed());
    },
    ...
    cooldown(iconDiv, duration) {
      ...
      const callback = timestamp => {
        ...
        if (progress >= 100) {
          ...
          iconDiv.style.background = 'linear-gradient(#fff ' + progress + '%, #808080 ' + progress + '%)';
    ...
    initWebSocket() {
      this.webSocket = new WebSocket(http.defaults.baseWsURL + '/controlPaneMaster');
