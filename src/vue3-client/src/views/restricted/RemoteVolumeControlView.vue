        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="(client, index) in clients" :key="index">
              ...
              <td style="padding: 0.5rem; padding-left: 0px; padding-right: 0px">
                <div class="btn-group btn-group-sm" role="group" aria-label="First group">
                  <button
                    type="button"
                    class="btn btn-secondary"
                    :disabled="client.value < 1"
                    v-on:click="updateDevice(client, (client.value = Math.max(0, client.value - 5)))"
                  >
                    -5
                  </button>
                  ...
                  <button
                    type="button"
                    class="btn btn-secondary"
                    :disabled="client.value > 99"
                    v-on:click="updateDevice(client, (client.value = Math.min(100, client.value - -5)))"
                  >
                    +5
                  </button>
                </div>
...
export default {
  ...
  methods: {
    ...
    initWebSocket() {
      this.webSocket = new WebSocket(http.defaults.baseWsURL + '/volumeMaster');
